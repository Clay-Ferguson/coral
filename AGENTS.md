...
- Dependencies: `python3-nautilus`, `zenity` (for dialogs), `gnome-terminal`, `pdftotext` (optional, from `poppler-utils` for PDF search), `ugrep` (optional, for the interactive Search (UGREP) TUI).
...
- **Search** shows on folders/background. Prompts for search term via zenity, generates temporary bash script at `/tmp/coral/coral-search-script.sh` that uses `grep -i` for regular files, `pdftotext | grep` for PDFs, and `find -iname` for filename/folder matching. Search runs in `gnome-terminal` showing live progress feedback. Once search completes, results are saved to `/tmp/coral/coral-search-results.txt` and a separate detached script (`/tmp/coral/coral-zenity-loop.sh`) is launched via `nohup` to handle the zenity results dialog—the terminal then exits naturally, leaving only the zenity GUI visible. User can select files to open—the list stays open for multiple selections. Markdown/text files (`.md`, `.txt`) open in VS Code; other files open with `xdg-open` (system default app). Content search respects user's mode choice (literal/basic regex/extended regex) via grep flags; filename search always uses literal matching.
- **Search (UGREP)** shows on folders/background, directly below Search. Launches ugrep's interactive TUI (`ugrep -Q -% --files -r -i .`) in `gnome-terminal` with the folder as working directory (`search_ugrep.py`); user types the pattern live in the TUI, no zenity involved. `-%` = Boolean query mode: `"quoted phrases"` are exact/literal, space/AND = all terms, OR = any, NOT/`-` = exclude, unquoted terms are regex. `--files` = queries match at whole-file scope, not per line. Honors config `search.included`/`search.excluded` patterns via ugrep `-g` globs (find-style `*/name/*` exclusions convert to `!name/`). Searches PDF content via `--filter='pdf:pdftotext -q % -'` when pdftotext is installed (ugrep executes filters directly, not via shell—no redirection/pipes allowed; no caching, unlike the ripgrep search).
