...
- **Search** shows on folders/background. Prompts for search term via zenity, generates temporary bash script at `/tmp/coral/coral-search-script.sh` that uses `grep -i` for regular files, `pdftotext | grep` for PDFs, and `find -iname` for filename/folder matching. Results are collected into a bash array and displayed in an interactive zenity pick list GUI. User can select files to open—the list stays open for multiple selections. Markdown/text files (`.md`, `.txt`) open in VS Code; other files open with `xdg-open` (system default app). Search runs in `gnome-terminal` for live progress feedback. Content search respects user's mode choice (literal/basic regex/extended regex) via grep flags; filename search always uses literal matching.
