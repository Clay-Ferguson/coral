...
- **Search** shows on folders/background. Prompts for search term via zenity, generates temporary bash script at `/tmp/coral/coral-search-script.sh` that uses `grep -i` for regular files, `pdftotext | grep` for PDFs, and `find -iname` for filename/folder matching. Results written to timestamped `/tmp/coral/coral-search--YYYY-MM-DD--HH-MM-SS.md` with three sections (Regular Files, PDF Files, Files and Folders) using clickable `file://` URLs and opened in VS Code. Search runs in `gnome-terminal` for live progress feedback. Content search respects user's mode choice (literal/basic regex/extended regex) via grep flags; filename search always uses literal matching.
